...
const CubiculosProfesores = () => {
  ...
  const [isEditingProfesor, setIsEditingProfesor] = useState(false);
  const [confirmDeleteId, setConfirmDeleteId] = useState(null);
  const [confirmDeleteProfesorId, setConfirmDeleteProfesorId] = useState(null);
  ...
  const handleDeleteProfesor = async () => {
    try {
      ...
      const response = await fetch(`https://mapaback.onrender.com/profesores/${confirmDeleteProfesorId}`, {
        method: 'DELETE',
        headers: getHeaders()
      });
      ...
      setConfirmDeleteProfesorId(null);
    } catch (err) {
    ...
    <div className="cubiculos-container">
      ...
      <div className="cubiculos-header">
        ...
        <div className="cubiculos-actions">
          <button onClick={handleAddProfesor} className="cubiculos-secondary-button">
              ...
              <div className="cubiculos-field-group">
                ...
                <div className="cubiculos-field-hint">
                  <button 
                    ...
                    className="cubiculos-text-button"
                  >
      ...
      {showProfesorForm && (
        <div className="cubiculos-modal-overlay">
          <div className="cubiculos-modal-content">
            <div className="cubiculos-modal-header">
              <h2 className="cubiculos-modal-title">
                {isEditingProfesor ? '✏️ Editar Profesor' : '➕ Registrar Nuevo Profesor'}
              </h2>
              <button 
            ...
            <div className="cubiculos-form-container">
              ...
              <div className="cubiculos-field-group">
                <label className="cubiculos-field-label">Nombre Completo *</label>
                <input
              ...
              <div className="cubiculos-field-group">
                <label className="cubiculos-field-label">Turno *</label>
                <select
                  value={profesorFormData.turno}
                  onChange={(e) => setProfesorFormData({ ...profesorFormData, turno: e.target.value })}
                  className="cubiculos-input"
                >
              ...
              {!isEditingProfesor && (
                <div className="cubiculos-field-group">
                  <label className="cubiculos-field-label">Código (opcional)</label>
                  <input
                    placeholder="Dejar en blanco para generar automáticamente"
                    value={profesorFormData.codigo}
                    onChange={(e) => setProfesorFormData({ ...profesorFormData, codigo: e.target.value })}
                    className="cubiculos-input"
                  />
                </div>
              )}
            ...
            {profesores.length > 0 && (
              <div className="cubiculos-profesores-list">
                ...
                <div className="cubiculos-list-container">
                  {profesores.map(profesor => (
                    <div key={profesor.codigo} className="cubiculos-profesor-item">
                      <div className="cubiculos-profesor-info">
                        <span className="cubiculos-profesor-nombre">{profesor.nombre}</span>
                        <span 
                          className="cubiculos-profesor-turno"
                          style={{ color: getTurnoColor(profesor.turno) }}
                        >
                          {profesor.turno}
                        </span>
                      </div>
                      <div className="cubiculos-profesor-actions">
                        <button 
                          onClick={() => handleEditProfesor(profesor)}
                          className="cubiculos-small-button"
                        >
                          ✏️
                        </button>
                        <button 
                          onClick={() => setConfirmDeleteProfesorId(profesor.codigo)}
                          className="cubiculos-small-danger-button"
                        >
                          🗑️
                        </button>
                      </div>
            ...
            <div className="cubiculos-modal-footer">
              <button onClick={() => setShowProfesorForm(false)} className="cubiculos-cancel-button">
                Cancelar
              </button>
              <button onClick={handleSubmitProfesor} className="cubiculos-primary-button">
                {isEditingProfesor ? '💾 Actualizar' : '💾 Registrar'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="cubiculos-modal-overlay">
          <div className="cubiculos-confirm-modal">
            ...
            <p className="cubiculos-confirm-message">
              ...
              Los cubículos asignados a este profesor quedarán sin asignar.
            </p>
            <div className="cubiculos-confirm-buttons">
              <button 
                onClick={() => setConfirmDeleteProfesorId(null)} 
                className="cubiculos-cancel-button"
              >
                Cancelar
              </button>
              <button onClick={handleDeleteProfesor} className="cubiculos-danger-button">
                Eliminar
              </button>
            </div>
          </div>
